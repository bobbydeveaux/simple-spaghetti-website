...
class PredictionSignal(BaseModel):
    """
    Prediction signal from the prediction engine.

    Represents a trading signal generated by analyzing technical indicators
    including RSI, MACD, and order book imbalance. Signals can be UP, DOWN, or SKIP.
    """
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "btc_price": 45678.90,
                "timestamp": "2024-01-15T10:30:00Z",
                "reasoning": "RSI oversold, MACD bullish crossover, strong buying pressure"
            }
        }
    )
    ...
    signal: SignalType = Field(
        ...,
        description="Prediction signal type (UP, DOWN, or SKIP)"
    )
    ...
    rsi: Optional[Decimal] = Field(
        default=None,
        description="RSI value at signal generation time",
        ge=0,
        le=100
    )
    ...
    order_book_imbalance: Optional[Decimal] = Field(
        default=None,
        description="Order book imbalance ratio (bid_volume / ask_volume)",
        ge=0
    )

    # Market Context
    btc_price: Optional[Decimal] = Field(
        default=None,
        description="BTC price at signal generation time",
        gt=0
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Serialize prediction signal to dictionary.

        Returns:
            Dictionary representation of the prediction signal
        """
        return {
    ...
    def is_actionable(self) -> bool:
        """Check if the signal is actionable (not SKIP)."""
        return self.signal != SignalType.SKIP

    def get_direction(self) -> Optional[str]:
        """Get the trading direction for actionable signals."""
        if self.signal == SignalType.UP:
            return "UP"
        elif self.signal == SignalType.DOWN:
            return "DOWN"
        return None
